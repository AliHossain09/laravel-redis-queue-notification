...
<p class="mt-1 text-xs uppercase tracking-[0.2em] text-amber-700">{{ $message['source'] ?? 'live' }}</p>
...
<form method="POST" action="/pubsub/messages/{{ $message['id'] }}" class="mt-3 space-y-2">
@csrf
@method('PATCH')
<input
type="text"
...
<summary class="cursor-pointer text-xs font-semibold uppercase tracking-[0.2em] text-slate-700">Edit Post</summary>
<form method="POST" action="/posts/{{ $post->id }}" class="mt-3 space-y-2">
@csrf
@method('PATCH')
<input
type="text"
...
<button class="rounded-lg bg-slate-900 px-3 py-2 text-xs font-semibold text-white">Update</button>
</form>
</details>
